...
### スイッチのルーティングを有効化する
スイッチのルーティングを有効化しないとレイヤ3のルーティングは行ってくれない。
グローバルコンフィギュレーションモードで[ip routing]コマンドを実行する。
```
...
Bluetooth Connection:
   ...
   Default Gateway.................: ::
                                     0.0.0.0
```
[1/0/2]につながれているパソコンのIPアドレスの設定も行う。

[IPv4 Address]を[172.1.2.1]に、[Subnet Mask]を[255.255.255.0]に、[Default Gateway]を[172.1.2.254]に設定する。
...
FastEthernet0 Connection:(default port)
   ...
   Connection-specific DNS Suffix..: 
   Link-local IPv6 Address.........: FE80::201:42FF:FE20:BAA0
   IPv6 Address....................: ::
   IPv4 Address....................: 172.1.2.1
   Subnet Mask.....................: 255.255.255.0
   Default Gateway.................: ::
                                     172.1.2.254
...
## スイッチの確認を行う
[show ip interface brief]を使用してルーテッドポートの正常性確認を行う。
```
Switch#show ip interface brief 
Interface              IP-Address      OK? Method Status                Protocol 
GigabitEthernet1/0/1   172.1.1.254     YES manual up                    up 
GigabitEthernet1/0/2   172.1.2.254     YES manual up                    up 
GigabitEthernet1/0/3   unassigned      YES unset  down                  down 
GigabitEthernet1/0/4   unassigned      YES unset  down                  down 
(以下略)
```
[GigabitEthernet1/0/1]と[GigabitEthernet1/0/2]のIPアドレスが正しいか、[OK?]が[YES]となっているか、[Status]と[Protocol]が[up]となっているかを確認する。

次にルーティングテーブルを確認するために[show ip route]コマンドを実行する。
```
...
C       172.1.1.0/24 is directly connected, GigabitEthernet1/0/1
L       172.1.1.254/32 is directly connected, GigabitEthernet1/0/1
C       172.1.2.0/24 is directly connected, GigabitEthernet1/0/2
L       172.1.2.254/32 is directly connected, GigabitEthernet1/0/2
```
最後の四行が重要。ルーティングテーブルは最初に一文字のアルファベットが表記され、その後に詳細な情報が表示される。[C]はconnectedの意味でインターフェイスに対して直接つながっていることを表す。[L]はLocal routeの意味で自分自身のインターフェイスに設定されているアドレスを表す。

例えば[C       172.1.1.0/24 is directly connected, GigabitEthernet1/0/1]だと、[172.1.1.0/24]は[GigabitEthernet1/0/1]に繋がっているという意味。
[L       172.1.1.254/32 is directly connected, GigabitEthernet1/0/1]の場合だと、[172.1.1.254/32]は[GigabitEthernet1/0/1]のIPアドレスであるということを示す。

ルーテッドポートの正常性確認とルーティングテーブルの確認ができたので、これで二つのPCは疎通できるはずである。

## pingコマンドを使って疎通確認を行う
追加した二つのPCのうちどちらかのコマンドプロンプトを開き、pingを打つ。例えば[172.1.1.1]のPCから[172.1.2.1]のPCへとpingを打つ場合は[ping 172.1.2.1]とコマンドを打ち込む。

結果は以下のようになる。
```
C:\>ping 172.1.2.1

Pinging 172.1.2.1 with 32 bytes of data:

Reply from 172.1.2.1: bytes=32 time<1ms TTL=127
Reply from 172.1.2.1: bytes=32 time<1ms TTL=127
Reply from 172.1.2.1: bytes=32 time<1ms TTL=127
Reply from 172.1.2.1: bytes=32 time<1ms TTL=127

Ping statistics for 172.1.2.1:
    Packets: Sent = 4, Received = 4, Lost = 0 (0% loss),
Approximate round trip times in milli-seconds:
    Minimum = 0ms, Maximum = 0ms, Average = 0ms
```
疎通ができているので、スイッチは[172.1.1.0/24]と[172.1.2.0]のルーティングができたということが分かった。
